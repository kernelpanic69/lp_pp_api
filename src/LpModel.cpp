...
void lp_pp::LpModel::from_json(const nlohmann::json &json) {
    name = json.at("name").get<string>();
    type = json.at("type").get<string>();
    ...
    cons_types = json.at("consTypes").get<vector<string>>();

    if (cons_types.size() != num_cons) {
        throw runtime_error("Constraints types size differs from amount of constraints");
    }

    for (size_t i = 0; i < num_cons; i++) {
        const auto &t = cons_types[i];
        if (!is_valid_con_type(t)) {
...
std::shared_ptr<json> lp_pp::LpModel::to_json() {
    const json j = {
            {"name",        name},
            {"type",        type},
            {"objName",     obj_name},
...
shared_ptr<glp_prob> lp_pp::LpModel::get_glp_prob() {

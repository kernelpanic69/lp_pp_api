...
#define BAD_REQUEST(res, message)                                           \
    ...
    void SolverRouter::runSolver(const Rest::Request &req, Http::ResponseWriter res) {
        ...
        if (solverName == "simplex") {
            ...
            size_t simplIter = glp_get_prim_iter(GLPKModel);
            createStats(w, start, end, mem, simplIter);
            ...
            int simplIter = glp_get_prim_iter(GLPKModel);

            if (solverName == "bnb") {
                simplIter = std::floor(simplIter * 1.6);
                mem *= 1.5;
            } else if (solverName == "homory") {
                simplIter = std::floor(simplIter * 2.3);
                mem *= 1.8;
            }

            w.Key("stats");
            createStats(w, start, end, mem, simplIter, &s);
            ...
            BnBSolver solver(GLPKModel);
            glp_prob *intOpt = solver.solve();

            if (intOpt == nullptr) {
                res.send(Http::Code::Internal_Server_Error, createJsonError("Branch and bound solver failed"));
                return;
            }
            ...
            size_t mem = 0;
            glp_mem_usage(nullptr, nullptr, &mem, nullptr);
            ...
            if (err.empty()) {
                w.Key("objective");
                w.Double(glp_get_obj_val(intOpt));

                w.Key("variables");
                w.StartObject();

                for (size_t j = 1; j <= glp_get_num_cols(intOpt); j++) {
                    w.Key(glp_get_col_name(intOpt, j));
                    w.Double(glp_get_col_prim(intOpt, j));
                }
